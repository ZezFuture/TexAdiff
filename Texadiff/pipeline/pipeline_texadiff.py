...
class StableDiffusionXLTexADiffPipeline(
    DiffusionPipeline,
    ...
    def __call__(
        ...
        **kwargs,
        ...
        self._num_timesteps = len(timesteps)
        if self.do_classifier_free_guidance:
            _, init_mask = mask.chunk(2)
        else:
            init_mask = mask

        num_steps = int(self._num_timesteps)
        lo = int(num_steps * (1-0.5) + 1 ) # i > 0.*T0.5
        hi = int(num_steps * (1-0.1) )  # i < 0.4*T0.1
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if (lo < i < hi) and (i & 1 == 0):
                    latents = prev_latents
                    latents.lerp_(new_latents, init_mask)
                else:
                    latents = new_latents
